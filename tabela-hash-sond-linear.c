...
void insert(HASHTABLE *table, char *key, char *name, int age, int rg)
{
    ...
    int collision = 0;
    while (table->data[linearhash(key, collision)] != NULL)
    {
        collision++;
    }
    table->data[linearhash(key, collision)] = newItem;
    table->length++;
}
...
ITEM *findElementByKey(HASHTABLE table, char *key)
{
    ...
    {
        ITEM *item = table.data[linearhash(key, collision)];
...
void removeElementByKey(HASHTABLE *table, char *key)
{
    ...
    while (collision < MAXSIZE)
    {
        ...
        if (item != NULL && strcmp(item->key, key) == 0)
        {
            table->data[linearhash(key, collision)] = NULL;
            table->length--;
            free(item);
            return;
        }
